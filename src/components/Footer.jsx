    ...
    <section className={`${styles.flexCenter}${styles.paddingY} flex-col`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex flex-col justify-start mr-10">
          ...
          />
          <p className={`${styles.paragraph} mt-4 max-w-[310px] text-gray-500`}>
          ...
          {footerLinks.map((footerLinks, key) => (
            <div key={key} className="flex flex-col sm:my-0 my-4 min-w-[150px]">
              ...
              <ul className="list-none mt-4">
                {footerLinks.links.map((linke, index) => (
                  <li
                    key={linke.name}
                    className={`font-poppins font-normal text-[16px] leading-[24px] text-gray-400 hover:text-[#6B7280] cursor-pointer ${
